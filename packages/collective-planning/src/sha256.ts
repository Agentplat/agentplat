/* Browser-safe synchronous SHA-256, deliberately free of runtime imports. */
...
export function sha256HexPlanningV1(bytes: Uint8Array): string {
  ...
  for (let offset = 0; offset < data.length; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      const s1 = rotateRight(ee, 6) ^ rotateRight(ee, 11) ^ rotateRight(ee, 25);
      const choice = (ee & ff) ^ (~ee & gg);
      const first = (hh + s1 + choice + K[index] + words[index]) >>> 0;
      const s0 = rotateRight(aa, 2) ^ rotateRight(aa, 13) ^ rotateRight(aa, 22);
