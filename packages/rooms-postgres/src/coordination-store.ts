...
/** PostgreSQL coordination store with ready-state and Room-scoped discovery. */
export class PostgresAgentRoomCoordinationStore implements AgentRoomCoordinationStore {
  ...
  constructor(
    ...
    this.table = `${quotePostgresIdentifier(schema)}.room_coordination_state`;
  }
  async load(tenantId: string, roomId: string, coordinationId: string) {
    const result = await this.pool.query<{ state: AgentRoomCoordinationState }>(
      `SELECT state FROM ${this.table} WHERE tenant_id=$1 AND room_id=$2 AND coordination_id=$3`,
  ...
  async list(tenantId: string, roomId: string) {
    const result = await this.pool.query<{ state: AgentRoomCoordinationState }>(
      `SELECT state FROM ${this.table} WHERE tenant_id=$1 AND room_id=$2 ORDER BY updated_at,coordination_id`,
      [tenantId, roomId],
    );
