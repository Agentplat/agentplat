...
export function createMorphogenesisSynthesisRestrictionAssessmentV5(
  input: Omit<
  ...
  const body = freeze({
    ...
    disposition: one(
      input.disposition,
      DISPOSITIONS,
      "synthesis restriction disposition",
    ),
...
export class MorphogenesisSynthesisEligibilityGateV5 {
  ...
  async evaluate(input: {
    ...
    const reasonCodes = assessments
      ...
      .map(({ source, disposition }) => `${source}_${disposition}`)
      .sort();
    ...
    const body = freeze({
      ...
      disposition: disposition as "eligible" | "ineligible",
...
export function validateMorphogenesisSynthesisEligibilityDecisionV5(
  ...
  if (
    ...
    value.candidateDigest !== input.candidate.candidateDigest ||
    ...
    decisionDigest !==
      digest("morphogenesis-synthesis-eligibility-decision-v5", body)
  )
...
function validateAssessment(
  value: MorphogenesisSynthesisRestrictionAssessmentV5,
  ...
  if (
    value.schemaVersion !== 5 ||
    rebuilt.assessmentDigest !== assessmentDigest ||
    rebuilt.source !== source ||
    rebuilt.candidateDigest !== input.candidate.candidateDigest ||
...
function id(value: unknown): AgentPlatID {
  if (typeof value !== "string" || !ID.test(value))
    fail("synthesis integration ID is invalid");
  return value as AgentPlatID;
}
function sha(value: unknown): PlanningDigestV1 {
  if (typeof value !== "string" || !SHA.test(value))
    fail("synthesis integration digest is invalid");
  return value as PlanningDigestV1;
}
function positive(value: unknown) {
  if (!Number.isSafeInteger(value) || (value as number) < 1)
    fail("synthesis integration positive integer is invalid");
  return value as number;
}
function nonNegative(value: unknown) {
  if (!Number.isSafeInteger(value) || (value as number) < 0)
    fail("synthesis integration time is invalid");
  return value as number;
}
...
function shas(values: readonly unknown[], minimum: number, maximum: number) {
  if (!Array.isArray(values)) fail("synthesis integration digests are invalid");
  ...
  if (
    ...
    fail("synthesis integration digest set is invalid");
