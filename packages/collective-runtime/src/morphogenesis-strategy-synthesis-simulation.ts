...
export interface MorphogenesisSynthesisSimulationStoreV5 {
  load(
    reportId: AgentPlatID,
  ): Promise<MorphogenesisSynthesisSimulationReportV5 | null>;
  save(report: MorphogenesisSynthesisSimulationReportV5): Promise<boolean>;
}

export function createMorphogenesisSynthesisSimulationScenarioV5(
  input: Omit<
    MorphogenesisSynthesisSimulationScenarioV5,
    "schemaVersion" | "candidateDigest" | "scenarioDigest"
  > & {
    readonly candidate: MorphogenesisStrategySynthesisCandidateV5;
...
export class InMemoryMorphogenesisSynthesisSimulationStoreV5 implements MorphogenesisSynthesisSimulationStoreV5 {
  ...
  async save(report: MorphogenesisSynthesisSimulationReportV5) {
    ...
    this.#reports.set(report.reportId, freeze(structuredClone(report)));
    return true;
  }
}
...
export class MorphogenesisSynthesisSimulationRuntimeV5 {
  ...
  constructor(
    ...
    if (!options?.simulator || !options?.store)
      fail("synthesis simulation ports are required");
  }
  async evaluate(input: {
    ...
    readonly report: MorphogenesisSynthesisSimulationReportV5;
    ...
    if (retained) {
      ...
      return {
        report,
        evaluation: evaluationFrom(report, input, candidate, this.#policy),
      };
    }
    if (
      scenario.expiresAtLogicalMs <= input.logicalTimeMs ||
    ...
    const result = await this.options.simulator.evaluate({
      scenario,
      candidate,
    });
...
export function validateMorphogenesisSynthesisSimulationReportV5(
  ...
  if (
    ...
    reportDigest !==
      digest("morphogenesis-synthesis-simulation-report-v5", body)
  )
...
function createReport(
  reportId: AgentPlatID,
  scenario: MorphogenesisSynthesisSimulationScenarioV5,
  result: MorphogenesisSynthesisSimulationResultV5,
) {
  if (
    result.scenarioDigest !== scenario.scenarioDigest ||
    result.candidateDigest !== scenario.candidateDigest ||
    result.simulatorImplementationDigest !==
      scenario.simulatorImplementationDigest ||
  ...
  const body = freeze({
    ...
    candidateDigest: scenario.candidateDigest,
    ...
    threatAssessments: freeze(
      result.threatAssessments
        .map(createMorphogenesisSynthesisThreatAssessmentV5)
        .sort((a, b) => a.threat.localeCompare(b.threat)),
    ),
    safetyMicros: micros(result.safetyMicros),
    confidenceBps: bps(result.confidenceBps),
    evidenceDigests: shas(result.evidenceDigests, 1, 256),
    interactionUnits: nonNegative(result.interactionUnits),
    evaluatedAtLogicalMs: nonNegative(result.completedAtLogicalMs),
...
function evaluationFrom(
  report: MorphogenesisSynthesisSimulationReportV5,
  input: {
    readonly evaluationId: AgentPlatID;
    readonly assessorId: AgentPlatID;
    readonly assessorImplementationDigest: PlanningDigestV1;
  ...
  return createMorphogenesisStrategySynthesisEvaluationV5({
    ...
    counterfactualReportDigest: report.reportDigest,
    assessorId: input.assessorId,
    assessorImplementationDigest: input.assessorImplementationDigest,
    threatAssessments: report.threatAssessments,
    safetyMicros: report.safetyMicros,
    confidenceBps: report.confidenceBps,
    evidenceDigests: [report.reportDigest, ...report.evidenceDigests],
    evaluatedAtLogicalMs: report.evaluatedAtLogicalMs,
    expiresAtLogicalMs: input.expiresAtLogicalMs,
    policy,
  });
}
function validateScenario(
  value: MorphogenesisSynthesisSimulationScenarioV5,
  candidate: MorphogenesisStrategySynthesisCandidateV5,
  policy: MorphogenesisStrategySynthesisPolicyV5,
) {
  const {
  ...
  if (scenarioDigest !== rebuilt.scenarioDigest)
    fail("synthesis simulation scenario is invalid");
  return rebuilt;
}
...
function id(value: unknown): AgentPlatID {
  if (typeof value !== "string" || !ID.test(value))
    fail("synthesis simulation ID is invalid");
  return value as AgentPlatID;
}
function sha(value: unknown): PlanningDigestV1 {
  if (typeof value !== "string" || !SHA.test(value))
    fail("synthesis simulation digest is invalid");
  return value as PlanningDigestV1;
}
function positive(value: unknown) {
  if (!Number.isSafeInteger(value) || (value as number) < 1)
    fail("synthesis simulation positive integer is invalid");
  return value as number;
}
function nonNegative(value: unknown) {
  if (!Number.isSafeInteger(value) || (value as number) < 0)
    fail("synthesis simulation non-negative integer is invalid");
  return value as number;
}
function bps(value: unknown) {
  const result = nonNegative(value);
  if (result > 10_000) fail("synthesis simulation confidence is invalid");
  return result;
}
function micros(value: unknown) {
  const result = nonNegative(value);
  if (result > 1_000_000) fail("synthesis simulation metric is invalid");
  return result;
}
function shas(values: readonly unknown[], minimum: number, maximum: number) {
  ...
  if (
    ...
    fail("synthesis simulation digests are invalid");
