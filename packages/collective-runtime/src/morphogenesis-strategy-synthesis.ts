...
export function createMorphogenesisStrategySynthesisPolicyV5(
  ...
  if (input.schemaVersion !== 5)
    fail("Morphogenesis synthesis policy version is invalid");
  const requiredThreats = enums(
    input.requiredThreats,
    new Set(MORPHOGENESIS_SYNTHESIS_THREATS_V5),
    "required synthesis threats",
  );
  if (requiredThreats.length !== MORPHOGENESIS_SYNTHESIS_THREATS_V5.length)
    fail(
      "Morphogenesis synthesis policy must require every adversarial threat",
    );
  const body = freeze({
    ...
    admittedSynthesizerImplementationDigests: shas(
      input.admittedSynthesizerImplementationDigests,
      1,
      32,
    ),
    requiredThreats,
...
export function createMorphogenesisStrategyGapV5(
  input: Omit<
    MorphogenesisStrategyGapV5,
    "schemaVersion" | "synthesisRequired" | "advisoryOnly" | "gapDigest"
  > & { readonly policy: MorphogenesisStrategySynthesisPolicyV5 },
): MorphogenesisStrategyGapV5 {
  ...
  const collectiveEvidenceDigests = shas(
    input.collectiveEvidenceDigests,
    1,
    128,
  );
  const evidenceDigests = shas(
    input.evidenceDigests,
    policy.minimumGapEvidence,
    256,
  );
  const expectedEvidence = [
    ...new Set([...localEvidenceDigests, ...collectiveEvidenceDigests]),
  ].sort();
  ...
  if (
    !same(evidenceDigests, expectedEvidence) ||
  ...
  const body = freeze({
    schemaVersion: 5 as const,
    gapId: id(input.gapId),
    catalogDigest: sha(input.catalogDigest),
    governanceStateDigest: sha(input.governanceStateDigest),
    contextDigest: sha(input.contextDigest),
    baselineStrategyId: id(input.baselineStrategyId),
    evaluatedStrategyIds,
    eligibleStrategyIds,
    localEvidenceDigests,
    collectiveEvidenceDigests,
    evidenceDigests,
    ...
    synthesisRequired: true as const,
    advisoryOnly: true as const,
  });
  if (
    body.catalogDigest !== policy.catalogDigest ||
    body.expiresAtLogicalMs <= body.detectedAtLogicalMs ||
    body.expiresAtLogicalMs - body.detectedAtLogicalMs >
      policy.maximumCandidateTtlMs
  )
    fail("Morphogenesis synthesis gap binding or window is invalid");
  return freeze({
    ...body,
    gapDigest: digest("morphogenesis-strategy-gap-v5", body),
  });
}

export function createMorphogenesisSynthesizedStrategyManifestV5(
  input: Omit<
    MorphogenesisSynthesizedStrategyManifestV5,
    "schemaVersion" | "manifestDigest"
  >,
): MorphogenesisSynthesizedStrategyManifestV5 {
  const body = freeze({
    schemaVersion: 5 as const,
    ...
    toolSetDigest: sha(input.toolSetDigest),
    memoryScopeDigest: sha(input.memoryScopeDigest),
    ...
    tokenBudget: positive(input.tokenBudget),
    toolCallBudget: nonNegative(input.toolCallBudget),
    maximumSpawnDepth: nonNegative(input.maximumSpawnDepth),
  });
  return freeze({
    ...body,
    manifestDigest: digest(
      "morphogenesis-synthesized-strategy-manifest-v5",
      body,
    ),
  });
}
...
export class MorphogenesisStrategySynthesisRuntimeV5 {
  ...
  constructor(
    ...
    this.#policy = validateMorphogenesisStrategySynthesisPolicyV5(
      options.policy,
    );
    if (
      !options?.synthesizer ||
      typeof options.synthesizer.synthesize !== "function"
    )
      fail("Morphogenesis synthesizer is required");
    if (
      !this.#policy.admittedSynthesizerImplementationDigests.includes(
  ...
  async synthesize(input: {
    readonly gap: MorphogenesisStrategyGapV5;
    readonly logicalTimeMs: number;
  }): Promise<MorphogenesisStrategySynthesisCandidateV5> {
...
export function createMorphogenesisSynthesisThreatAssessmentV5(
  input: Omit<
  ...
  return freeze({
    ...
    assessmentDigest: digest(
      "morphogenesis-synthesis-threat-assessment-v5",
      body,
    ),
  });
}

export function createMorphogenesisStrategySynthesisEvaluationV5(
  input: Omit<
    MorphogenesisStrategySynthesisEvaluationV5,
    "schemaVersion" | "disposition" | "advisoryOnly" | "evaluationDigest"
  > & {
    readonly policy: MorphogenesisStrategySynthesisPolicyV5;
  ...
  const body = freeze({
    ...
    threatAssessments: freeze(
      [...assessments].sort((a, b) => a.threat.localeCompare(b.threat)),
    ),
    ...
    disposition: (unsafe
      ...
      : inconclusive
        ? "inconclusive"
        : "eligible") as MorphogenesisStrategySynthesisEvaluationV5["disposition"],
  ...
  if (
    ...
    body.expiresAtLogicalMs - body.evaluatedAtLogicalMs >
      policy.maximumEvaluationTtlMs
  )
    fail("Morphogenesis synthesis evaluation window is invalid");
  return freeze({
    ...body,
    evaluationDigest: digest(
      "morphogenesis-strategy-synthesis-evaluation-v5",
      body,
    ),
  });
}

export function createMorphogenesisStrategySynthesisCertificationV5(
  input: Omit<
    MorphogenesisStrategySynthesisCertificationV5,
    "schemaVersion" | "grantsAuthority" | "certificationDigest"
  > & {
    readonly candidate: MorphogenesisStrategySynthesisCandidateV5;
  ...
  if (
    candidate.candidateDigest !== evaluation.candidateDigest ||
    ...
    fail(
      "Morphogenesis synthesis certification independence or evidence is invalid",
    );
  const body = freeze({
    schemaVersion: 5 as const,
    certificationId: id(input.certificationId),
    candidateDigest: candidate.candidateDigest,
    evaluationDigest: evaluation.evaluationDigest,
    synthesizerId: id(input.synthesizerId),
  ...
  if (
    body.expiresAtLogicalMs <= body.certifiedAtLogicalMs ||
  ...
  return freeze({
    ...
    certificationDigest: digest(
      "morphogenesis-strategy-synthesis-certification-v5",
      body,
    ),
  });
}

export function validateMorphogenesisStrategySynthesisPolicyV5(
  value: MorphogenesisStrategySynthesisPolicyV5,
) {
  const { policyDigest, ...body } = exact(
    value,
    POLICY_KEYS,
    "Morphogenesis synthesis policy",
  ) as unknown as MorphogenesisStrategySynthesisPolicyV5;
  const rebuilt = createMorphogenesisStrategySynthesisPolicyV5(body);
  if (policyDigest !== rebuilt.policyDigest)
    fail("Morphogenesis synthesis policy digest is invalid");
  return rebuilt;
}
export function validateMorphogenesisStrategyGapV5(
  value: MorphogenesisStrategyGapV5,
  policy: MorphogenesisStrategySynthesisPolicyV5,
) {
  const {
    gapDigest,
    schemaVersion: _s,
    synthesisRequired: _r,
    advisoryOnly: _a,
    ...body
  } = exact(
    value,
    GAP_KEYS,
    "Morphogenesis synthesis gap",
  ) as unknown as MorphogenesisStrategyGapV5;
  const rebuilt = createMorphogenesisStrategyGapV5({ ...body, policy });
  if (gapDigest !== rebuilt.gapDigest)
    fail("Morphogenesis synthesis gap digest is invalid");
  return rebuilt;
}
export function validateMorphogenesisStrategySynthesisCandidateV5(
  ...
  const record = exact(
    value,
    CANDIDATE_KEYS,
    "Morphogenesis synthesis candidate",
  ) as unknown as MorphogenesisStrategySynthesisCandidateV5;
  ...
  if (
    ...
    record.catalogDigest !== policy.catalogDigest ||
    candidateDigest !==
      digest("morphogenesis-strategy-synthesis-candidate-v5", rebuilt)
  )
...
export function validateMorphogenesisStrategySynthesisEvaluationV5(
  ...
  const record = exact(
    value,
    EVALUATION_KEYS,
    "Morphogenesis synthesis evaluation",
  ) as unknown as MorphogenesisStrategySynthesisEvaluationV5;
  const {
    schemaVersion: _s,
    disposition: _d,
    advisoryOnly: _a,
  ...
  if (
    ...
    record.disposition !== rebuilt.disposition ||
...
export function validateMorphogenesisStrategySynthesisCertificationV5(
  ...
  const record = exact(
    value,
    CERTIFICATION_KEYS,
    "Morphogenesis synthesis certification",
  ) as unknown as MorphogenesisStrategySynthesisCertificationV5;
  const {
    schemaVersion: _s,
    grantsAuthority: _g,
    certificationDigest,
  ...
  if (
    certificationDigest !== rebuilt.certificationDigest ||
...
function createCandidate(
  draft: Awaited<
    ReturnType<MorphogenesisStrategySynthesizerPortV5["synthesize"]>
  >,
  gap: MorphogenesisStrategyGapV5,
  policy: MorphogenesisStrategySynthesisPolicyV5,
  ...
  return freeze({
    ...
    candidateDigest: digest(
      "morphogenesis-strategy-synthesis-candidate-v5",
      body,
    ),
  });
}
function candidateBody(
  ...
  return freeze({
    ...
    gapDigest: sha(input.gapDigest),
    policyDigest: sha(input.policyDigest),
    catalogDigest: sha(input.catalogDigest),
    manifest,
    synthesizerId: id(input.synthesizerId),
    synthesizerVersion: positive(input.synthesizerVersion),
...
function validateManifest(value: MorphogenesisSynthesizedStrategyManifestV5) {
  const record = exact(
    value,
    MANIFEST_KEYS,
    "Morphogenesis synthesized strategy manifest",
  ) as unknown as MorphogenesisSynthesizedStrategyManifestV5;
  const { schemaVersion: _s, manifestDigest, ...body } = record;
  const rebuilt = createMorphogenesisSynthesizedStrategyManifestV5(body);
  if (manifestDigest !== rebuilt.manifestDigest)
    fail("Morphogenesis synthesis manifest digest is invalid");
  return rebuilt;
}
function assertManifestBudget(
  value: MorphogenesisSynthesizedStrategyManifestV5,
  policy: MorphogenesisStrategySynthesisPolicyV5,
) {
  if (
    value.tokenBudget > policy.maximumTokenBudget ||
...
const POLICY_KEYS = [
  ...
  "maximumCandidatesPerGap",
  "maximumEvaluationTtlMs",
  "maximumSpawnDepth",
  "maximumTokenBudget",
  "maximumToolCalls",
  "minimumEvaluationConfidenceBps",
  "minimumGapEvidence",
  "minimumSafetyMicros",
  "parentPolicyDigest",
  "policyDigest",
...
const MANIFEST_KEYS = [
  ...
  "profileEvolutionDigest",
  "proposalGeneratorDigest",
  "schemaVersion",
  "strategyId",
  "strategyImplementationDigest",
  "strategyVersion",
  "supportedOperators",
  "tokenBudget",
  "toolCallBudget",
...
function id(value: unknown): AgentPlatID {
  if (typeof value !== "string" || !ID.test(value))
    fail("Morphogenesis synthesis ID is invalid");
  return value as AgentPlatID;
}
function sha(value: unknown): PlanningDigestV1 {
  if (typeof value !== "string" || !SHA.test(value))
    fail("Morphogenesis synthesis digest is invalid");
...
function positive(value: unknown) {
  if (!Number.isSafeInteger(value) || (value as number) < 1)
    fail("Morphogenesis synthesis positive integer is invalid");
  return value as number;
}
function nonNegative(value: unknown) {
  if (!Number.isSafeInteger(value) || (value as number) < 0)
    fail("Morphogenesis synthesis non-negative integer is invalid");
  return value as number;
}
function bounded(value: unknown, minimum: number, maximum: number) {
  const result = nonNegative(value);
  if (result < minimum || result > maximum)
    fail("Morphogenesis synthesis bounded integer is invalid");
  return result;
}
function bps(value: unknown) {
  return bounded(value, 0, 10_000);
}
function micros(value: unknown) {
  return bounded(value, 0, 1_000_000);
}
function ids(values: readonly unknown[], minimum: number, maximum: number) {
  if (!Array.isArray(values)) fail("Morphogenesis synthesis IDs are invalid");
  ...
  if (
    ...
    fail("Morphogenesis synthesis ID set is invalid");
  return freeze(result);
}
function shas(values: readonly unknown[], minimum: number, maximum: number) {
  if (!Array.isArray(values))
    fail("Morphogenesis synthesis digests are invalid");
  ...
  if (
    ...
    fail("Morphogenesis synthesis digest set is invalid");
  return freeze(result);
}
...
function enums<T extends string>(
  ...
  if (result.length < 1 || result.length !== values.length)
    fail(`${label} are invalid`);
  return freeze(result);
}
function same(left: readonly unknown[], right: readonly unknown[]) {
  return (
