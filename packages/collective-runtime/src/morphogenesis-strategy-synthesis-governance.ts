...
export interface MorphogenesisSynthesisReviewPortV5 {
  ...
  }): Promise<MorphogenesisSynthesisAdmissionReviewV5 | null>;
}
export interface MorphogenesisSynthesisGovernanceStoreV5 {
  load(
    stateKey: AgentPlatID,
  ): Promise<MorphogenesisSynthesisGovernanceStateV5 | null>;
  save(input: {
    readonly state: MorphogenesisSynthesisGovernanceStateV5;
    readonly expectedRevision: number | null;
    readonly expectedStateDigest: PlanningDigestV1 | null;
  }): Promise<boolean>;
}

export function createMorphogenesisSynthesisGovernancePolicyV5(
  input: Omit<MorphogenesisSynthesisGovernancePolicyV5, "policyDigest">,
): MorphogenesisSynthesisGovernancePolicyV5 {
  if (input.schemaVersion !== 5)
    fail("synthesis governance policy version is invalid");
  const body = freeze({
  ...
  if (
    body.minimumCanaryOutcomes > body.maximumCanarySelections ||
...
export function createMorphogenesisSynthesisAdmissionReviewV5(
  input: Omit<
  ...
  const body = freeze({
    ...
    route: one(input.route, ROUTES, "synthesis review route"),
  ...
  return freeze({
    ...body,
    reviewDigest: digest("morphogenesis-synthesis-review-v5", body),
  });
}

export class InMemoryMorphogenesisSynthesisGovernanceStoreV5 implements MorphogenesisSynthesisGovernanceStoreV5 {
...
export class MorphogenesisSynthesisGovernanceRuntimeV5 {
  ...
  constructor(
    ...
    this.#synthesisPolicy = validateMorphogenesisStrategySynthesisPolicyV5(
      options.synthesisPolicy,
    );
    if (
      this.#policy.synthesisPolicyDigest !==
        this.#synthesisPolicy.policyDigest ||
      !options.reviews ||
      !options.store
    )
      fail("synthesis governance bindings are invalid");
  }
  ...
  async recommend(
    input: Omit<
      MorphogenesisSynthesisAdmissionRecommendationV5,
      | "schemaVersion"
      | "stateDigest"
      | "stateRevision"
      | "advisoryOnly"
    ...
    await this.#commit(input.proposedAtLogicalMs, (state) => {
      ...
      if (
        ...
        Math.min(
          entry.evaluationExpiresAtLogicalMs,
          entry.certificationExpiresAtLogicalMs,
          entry.eligibilityExpiresAtLogicalMs,
  ...
  async reviewAndApply(input: {
    ...
    await this.#commit(input.logicalTimeMs, async (state) => {
      ...
      const review = await this.options.reviews.review({
        recommendation,
        policy: this.#policy,
        logicalTimeMs: input.logicalTimeMs,
      });
      ...
      const nextStatus =
        normalized.disposition === "rejected"
          ? entry.status
      ...
      const entries = state.entries.map((item) =>
        item.candidate.candidateDigest === entry.candidate.candidateDigest
          ? freeze({
              ...item,
              status: nextStatus,
              statusRevision:
                item.statusRevision + (nextStatus === item.status ? 0 : 1),
      ...
      return nextState(state, input.logicalTimeMs, {
        ...
        reviews: boundedAppend(
          state.reviews,
          normalized,
          this.#policy.maximumHistory,
        ),
        transitions: boundedAppend(
          state.transitions,
          transition!,
          this.#policy.maximumHistory,
        ),
  ...
  async observeCanary(input: {
    ...
    return this.#commit(input.logicalTimeMs, (state) => {
      ...
      const retained = entry.canaryReceipts.find(
      ...
      const canary = freeze({
        ...entry.canary,
        selections: entry.canary.selections + 1,
      ...
      const entries = state.entries.map((item) =>
        item.candidate.candidateDigest === entry.candidate.candidateDigest
          ? freeze({
              ...item,
              canary,
              canaryReceipts: freeze([...item.canaryReceipts, receipt]),
            })
          : item,
      );
  ...
  async #load(logicalTimeMs: number) {
    ...
    return (
      retained ??
      initialState(this.options.stateKey, this.#policy, logicalTimeMs)
    );
  }
  async #commit(
    logicalTimeMs: number,
    mutate: (
      state: MorphogenesisSynthesisGovernanceStateV5,
    ) =>
      | MorphogenesisSynthesisGovernanceStateV5
      | Promise<MorphogenesisSynthesisGovernanceStateV5>,
  ) {
    for (
      let attempt = 0;
      ...
      if (logicalTimeMs < current.logicalTimeHighWaterMs)
        fail("synthesis governance time rolled back");
  ...
  const normalized = createRecommendation(
    {
      ...
      riskDigest: value.riskDigest,
      costDigest: value.costDigest,
    ...
    } as MorphogenesisSynthesisGovernanceStateV5,
  );
  if (
...
export function validateMorphogenesisSynthesisGovernanceStateV5(
  ...
  const synthesisPolicy = validateMorphogenesisStrategySynthesisPolicyV5(
    input.synthesisPolicy,
  );
  const record = exact(
    value,
    STATE_KEYS,
    "synthesis governance state",
  ) as unknown as MorphogenesisSynthesisGovernanceStateV5;
  if (
  ...
  const entries = record.entries.map((entry) =>
    validateEntry(entry, synthesisPolicy),
  );
  if (
    new Set(entries.map(({ candidate }) => candidate.candidateDigest)).size !==
    entries.length
  )
    fail("synthesis governance entries are duplicated");
  const pendingRecommendations = record.pendingRecommendations.map(
    (recommendation) => {
      const normalized = createRecommendation(
        {
          ...
          riskDigest: recommendation.riskDigest,
          costDigest: recommendation.costDigest,
  ...
  const rebuilt = stateBody({
    stateKey: id(record.stateKey),
...
function stateBody(
  ...
  return freeze({
    ...body,
    stateDigest: digest("morphogenesis-synthesis-governance-state-v5", body),
  });
}
function createRecommendation(
  ...
  const body = freeze({
    ...
    evaluationDigest: sha(input.evaluationDigest),
  ...
  return freeze({
    ...
    recommendationDigest: digest(
      "morphogenesis-synthesis-recommendation-v5",
      body,
    ),
  });
}
function createTransition(
  recommendation: MorphogenesisSynthesisAdmissionRecommendationV5,
  review: MorphogenesisSynthesisAdmissionReviewV5,
  priorStatus: MorphogenesisSynthesisCatalogEntryV5["status"],
  nextStatus: MorphogenesisSynthesisCatalogEntryV5["status"],
...
function createCanaryReceipt(input: {
  readonly observationId: AgentPlatID;
  readonly candidateDigest: PlanningDigestV1;
  readonly outcome: "success" | "failure" | "unsafe" | "indeterminate";
  readonly outcomeEvidenceDigest: PlanningDigestV1;
  ...
  const body = freeze({
    ...
    outcome: one(
      input.outcome,
      new Set<"success" | "failure" | "unsafe" | "indeterminate">([
        "success",
        "failure",
        "unsafe",
        "indeterminate",
      ]),
      "synthesis canary outcome",
    ),
...
function assertAction(
  entry: MorphogenesisSynthesisCatalogEntryV5,
  ...
  const allowed =
    action === "admit_experimental"
      ...
      : action === "certify"
        ...
        : action === "degrade"
          ? ["experimental", "certified"].includes(entry.status)
          : action === "retire"
...
function statusFor(
  ...
  return action === "admit_experimental" || action === "rollback"
    ? "experimental"
    : action === "certify"
      ? "certified"
      : action === "degrade"
...
function entryFor(
  state: MorphogenesisSynthesisGovernanceStateV5,
  digestValue: PlanningDigestV1,
) {
  const entry = state.entries.find(
    ({ candidate }) => candidate.candidateDigest === digestValue,
  );
  if (!entry) fail("synthesis candidate is not registered");
  return entry;
}
function emptyCanary(): MorphogenesisSynthesisCanaryCountersV5 {
  return freeze({
    selections: 0,
    successes: 0,
    failures: 0,
...
function validateEntry(
  value: MorphogenesisSynthesisCatalogEntryV5,
  policy: MorphogenesisStrategySynthesisPolicyV5,
) {
  const record = exact(
    value,
    ENTRY_KEYS,
    "synthesis governance entry",
  ) as unknown as MorphogenesisSynthesisCatalogEntryV5;
  const candidate = validateMorphogenesisStrategySynthesisCandidateV5(
    record.candidate,
    policy,
  );
  const receipts = record.canaryReceipts.map((receipt) => {
    const normalized = createCanaryReceipt({
    ...
    if (
      normalized.receiptDigest !== receipt.receiptDigest ||
  ...
  if (
    new Set(receipts.map(({ observationId }) => observationId)).size !==
    receipts.length
  )
    fail("synthesis canary receipts are duplicated");
  const canary = freeze({
    selections: receipts.length,
    ...
    indeterminate: receipts.filter(({ outcome }) => outcome === "indeterminate")
      .length,
  });
  ...
  return freeze({
    ...
    certificationExpiresAtLogicalMs: positive(
      record.certificationExpiresAtLogicalMs,
    ),
    eligibilityExpiresAtLogicalMs: positive(
      record.eligibilityExpiresAtLogicalMs,
    ),
    status: one(record.status, STATUSES, "synthesis lifecycle status"),
    statusRevision: positive(record.statusRevision),
    canary,
    canaryReceipts: freeze(receipts),
    lastTransitionDigest:
      record.lastTransitionDigest === null
        ? null
...
function validateTransition(
  ...
  const record = exact(
    value,
    TRANSITION_KEYS,
    "synthesis governance transition",
  ) as unknown as MorphogenesisSynthesisGovernanceTransitionV5;
  const { transitionDigest, ...body } = record;
  if (
    record.schemaVersion !== 5 ||
    transitionDigest !==
      digest("morphogenesis-synthesis-governance-transition-v5", body)
  )
...
function validateGovernancePolicy(
  ...
  if (policyDigest !== rebuilt.policyDigest)
    fail("synthesis governance policy digest is invalid");
  return rebuilt;
}
...
const ENTRY_KEYS = [
  ...
  "candidate",
  "certificationDigest",
  "certificationExpiresAtLogicalMs",
  "eligibilityDecisionDigest",
  "eligibilityExpiresAtLogicalMs",
  "evaluationDigest",
  "evaluationExpiresAtLogicalMs",
  "lastTransitionDigest",
  "schemaVersion",
  "status",
  "statusRevision",
...
function id(value: unknown): AgentPlatID {
  if (typeof value !== "string" || !ID.test(value))
    fail("synthesis governance ID is invalid");
  return value as AgentPlatID;
}
function sha(value: unknown): PlanningDigestV1 {
  if (typeof value !== "string" || !SHA.test(value))
    fail("synthesis governance digest is invalid");
  return value as PlanningDigestV1;
}
function positive(value: unknown) {
  if (!Number.isSafeInteger(value) || (value as number) < 1)
    fail("synthesis governance positive integer is invalid");
  return value as number;
}
function nonNegative(value: unknown) {
  if (!Number.isSafeInteger(value) || (value as number) < 0)
    fail("synthesis governance non-negative integer is invalid");
  return value as number;
}
function bool(value: unknown) {
  if (typeof value !== "boolean")
    fail("synthesis governance boolean is invalid");
  return value;
}
...
function shas(values: readonly unknown[], minimum: number, maximum: number) {
  if (!Array.isArray(values)) fail("synthesis governance digests are invalid");
  ...
  if (
    ...
    fail("synthesis governance digest set is invalid");
  return freeze(result);
}
