...
import type { MorphogenesisAgentGenesisDraftV6,
  MorphogenesisAgentGenesisSandboxReceiptV6 } from "./morphogenesis-agent-genesis.js";
...
export interface MorphogenesisAgentGenesisProbationAssessmentPortV6 {
  readonly source: MorphogenesisAgentGenesisProbationSourceV6;
  assess(input: { readonly draft: MorphogenesisAgentGenesisDraftV6;
    readonly sandboxReceipt: MorphogenesisAgentGenesisSandboxReceiptV6;
    readonly logicalTimeMs: number }): Promise<MorphogenesisAgentGenesisProbationAssessmentV6>;
}
...
export function createMorphogenesisAgentGenesisProbationAssessmentV6(input:
  Omit<MorphogenesisAgentGenesisProbationAssessmentV6, "schemaVersion" | "assessmentDigest">) {
  const body = freeze({ schemaVersion: 6 as const,
    source: source(input.source), draftDigest: sha(input.draftDigest),
    sandboxReceiptDigest: sha(input.sandboxReceiptDigest),
    disposition: disposition(input.disposition), policyDigest: sha(input.policyDigest),
  ...
  return freeze({ ...body, assessmentDigest: digest(
    "morphogenesis-agent-genesis-probation-assessment-v6", body) });
}

export class MorphogenesisAgentGenesisProbationEligibilityGateV6 {
  constructor(readonly options: { readonly capability: MorphogenesisAgentGenesisProbationAssessmentPortV6;
    ...
    if (options.capability.source !== "capability" || options.trust.source !== "trust" ||
        options.inferenceControl.source !== "inference_control")
      fail("Agent Genesis probation sources are invalid");
  }
  async evaluate(input: { readonly draft: MorphogenesisAgentGenesisDraftV6;
    readonly sandboxReceipt: MorphogenesisAgentGenesisSandboxReceiptV6;
    readonly logicalTimeMs: number }): Promise<MorphogenesisAgentGenesisProbationEligibilityV6> {
    const raw = await Promise.all([this.options.capability.assess(input),
      this.options.trust.assess(input), this.options.inferenceControl.assess(input)]);
    const assessments = raw.map((value, index) => validateAssessment(value,
      (["capability", "trust", "inference_control"] as const)[index]!, input));
    const reasonCodes = assessments.filter(({ disposition }) => disposition !== "eligible")
      .map(({ source, disposition }) => `${source}_${disposition}` as AgentPlatID).sort();
    const body = freeze({ schemaVersion: 6 as const, draftDigest: input.draft.draftDigest,
      ...
      disposition: (reasonCodes.length ? "ineligible" : "eligible") as "eligible" | "ineligible",
      reasonCodes: freeze(reasonCodes), evaluatedAtLogicalMs: nonNegative(input.logicalTimeMs),
      expiresAtLogicalMs: Math.min(...assessments.map(({ expiresAtLogicalMs }) =>
        expiresAtLogicalMs)), grantsAuthority: false as const });
    return freeze({ ...body, decisionDigest: digest(
      "morphogenesis-agent-genesis-probation-eligibility-v6", body) });
  }
}
...
export function validateMorphogenesisAgentGenesisProbationEligibilityV6(
  ...
  input: { readonly draft: MorphogenesisAgentGenesisDraftV6;
    readonly sandboxReceipt: MorphogenesisAgentGenesisSandboxReceiptV6;
    readonly logicalTimeMs: number }) {
  const { decisionDigest, ...body } = value;
  if (value.schemaVersion !== 6 || value.draftDigest !== input.draft.draftDigest ||
      ...
      decisionDigest !== digest("morphogenesis-agent-genesis-probation-eligibility-v6", body))
...
function sha(value: unknown): PlanningDigestV1 { if (typeof value !== "string" || !SHA.test(value)) fail("Agent Genesis integration digest is invalid"); return value as PlanningDigestV1; }
...
function shas(values: readonly unknown[]) { const result = [...new Set(values.map(sha))].sort(); if (!result.length || result.length !== values.length) fail("Agent Genesis integration evidence is invalid"); return freeze(result); }
function source(value: unknown): MorphogenesisAgentGenesisProbationSourceV6 { if (!new Set(["capability", "trust", "inference_control"]).has(value as string)) fail("Agent Genesis probation source is invalid"); return value as MorphogenesisAgentGenesisProbationSourceV6; }
function disposition(value: unknown): MorphogenesisAgentGenesisProbationAssessmentV6["disposition"] { if (!new Set(["eligible", "restricted", "denied", "unavailable"]).has(value as string)) fail("Agent Genesis probation disposition is invalid"); return value as MorphogenesisAgentGenesisProbationAssessmentV6["disposition"]; }
function validateAssessment(value: MorphogenesisAgentGenesisProbationAssessmentV6,
  ...
  if (normalized.assessmentDigest !== assessmentDigest || normalized.source !== expected ||
