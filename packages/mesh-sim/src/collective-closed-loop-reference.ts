...
export async function createCollectiveClosedLoopReferenceScenarioV1(
  ...
  const planningProposal = proposalFor(
    ...
    peerIds[0],
    peerInstanceId(0),
    input.runner === 'centralized_planner'
      ? initialObservations.slice(
          0,
  ...
  const registration = createCollectiveEvaluationRegistrationBindingV1({
    ...
    seed: SEED,
    environmentDigest:
      collectiveDeterministicEnvironmentDigestV1(environmentDefinition),
  ...
  return Object.freeze({
    ...
    async prepareAction(
      ...
      if (
        context.workContract.workContractDigest !== expectedWorkContractDigest ||
        context.winnerPeerId !== context.workContract.assignment.assignedPeerId ||
        context.logicalTimeMs !== actionLogicalTimeMs
      )
        throw new Error('closed_loop_reference_preflight_mismatch');
      return prepareActionFor(context);
    },
  });
}
...
function peerIdsFor(peerCount: number): readonly string[] {
  return Object.freeze(
    Array.from({ length: peerCount }, (_, index) =>
      `peer:${String(index).padStart(3, '0')}`,
    ),
  );
}
...
function decisionPolicyFor(
  ...
  return Object.freeze({
    ...
    policyVersion: 1,
    policyDigest,
    decide(context: Parameters<CollectiveClosedLoopExecutionInputV1['decisionPolicy']['decide']>[0]) {
...
function trustFor(work: WorkContractV1, logicalTimeMs: number) {
  ...
  const policy = createEvidenceFusionPolicyV1({
    ...
    sourceBindings: [
      {
        ...
        validUntilLogicalMs: MAXIMUM_LOGICAL_TIME_MS,
      },
    ],
    dependencyGroups: [
      {
    ...
    eligibilityRules: [
      {
        ruleId: 'permit',
        maximumProfileAgeMs: MAXIMUM_LOGICAL_TIME_MS,
        requirements: [
          {
            dimensionId: 'integrity',
...
function inferenceFor(
  ...
  const policy = {
    ...
    maximumRunDurationMs: MAXIMUM_LOGICAL_TIME_MS,
