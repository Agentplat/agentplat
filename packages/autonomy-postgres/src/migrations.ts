...
const applicationId = "@agentplat/autonomy-postgres";
const migrationName = "001_progressive_autonomy";

export const migrationDirectory = fileURLToPath(
  new URL("../migrations/", import.meta.url),
);

export interface AutonomyPostgresMigrationOptionsV1 {
  ...
  return [
    {
      ...
      adoptIf: `
        SELECT
          to_regclass('__AGENTPLAT_SCHEMA__.autonomy_policies') IS NOT NULL
          AND to_regclass('__AGENTPLAT_SCHEMA__.autonomy_states') IS NOT NULL
          AND to_regclass('__AGENTPLAT_SCHEMA__.autonomy_decisions') IS NOT NULL
          AS present
      `,
    },
  ] as const;
...
export async function runMigrations(
  pool: Pool,
  options: AutonomyPostgresMigrationOptionsV1 = {},
...
export async function rollbackMigrations(
  pool: Pool,
  input: AutonomyPostgresMigrationOptionsV1 & {
  ...
  if (input.verifiedBackup !== true)
    throw new TypeError(
      "Autonomy rollback requires a verified external backup",
    );
