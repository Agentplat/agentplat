/* Browser-safe synchronous SHA-256, deliberately free of runtime-specific imports. */
const K = new Uint32Array([
  0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1,
  0x923f82a4, 0xab1c5ed5, 0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3,
  0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174, 0xe49b69c1, 0xefbe4786,
  0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
  0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147,
  0x06ca6351, 0x14292967, 0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13,
  0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85, 0xa2bfe8a1, 0xa81a664b,
  0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
  0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a,
  0x5b9cca4f, 0x682e6ff3, 0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208,
  0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
]);
const rotr = (value: number, count: number) =>
  (value >>> count) | (value << (32 - count));
export function sha256Hex(bytes: Uint8Array): string {
  const bitLength = bytes.length * 8;
  if (!Number.isSafeInteger(bitLength))
    throw new RangeError("SHA-256 input is too large");
  const paddedLength = Math.ceil((bytes.length + 9) / 64) * 64;
  const data = new Uint8Array(paddedLength);
  data.set(bytes);
  data[bytes.length] = 0x80;
  const view = new DataView(data.buffer);
  view.setUint32(
    paddedLength - 8,
    Math.floor(bitLength / 0x1_0000_0000),
    false,
  );
  view.setUint32(paddedLength - 4, bitLength >>> 0, false);
  let a = 0x6a09e667,
    b = 0xbb67ae85,
    c = 0x3c6ef372,
    d = 0xa54ff53a,
    e = 0x510e527f,
    f = 0x9b05688c,
    g = 0x1f83d9ab,
    h = 0x5be0cd19;
  const words = new Uint32Array(64);
  for (let offset = 0; offset < data.length; offset += 64) {
    for (let i = 0; i < 16; i++)
      words[i] = view.getUint32(offset + i * 4, false);
    for (let i = 16; i < 64; i++) {
      const x = words[i - 15],
        y = words[i - 2];
      words[i] =
        ((rotr(x, 7) ^ rotr(x, 18) ^ (x >>> 3)) +
          words[i - 16] +
          (rotr(y, 17) ^ rotr(y, 19) ^ (y >>> 10)) +
          words[i - 7]) >>>
        0;
    }
    let aa = a,
      bb = b,
      cc = c,
      dd = d,
      ee = e,
      ff = f,
      gg = g,
      hh = h;
    for (let i = 0; i < 64; i++) {
      const s1 = rotr(ee, 6) ^ rotr(ee, 11) ^ rotr(ee, 25),
        choice = (ee & ff) ^ (~ee & gg),
        t1 = (hh + s1 + choice + K[i] + words[i]) >>> 0,
        s0 = rotr(aa, 2) ^ rotr(aa, 13) ^ rotr(aa, 22),
        majority = (aa & bb) ^ (aa & cc) ^ (bb & cc),
        t2 = (s0 + majority) >>> 0;
      hh = gg;
      gg = ff;
      ff = ee;
      ee = (dd + t1) >>> 0;
      dd = cc;
      cc = bb;
      bb = aa;
      aa = (t1 + t2) >>> 0;
    }
    a = (a + aa) >>> 0;
    b = (b + bb) >>> 0;
    c = (c + cc) >>> 0;
    d = (d + dd) >>> 0;
    e = (e + ee) >>> 0;
    f = (f + ff) >>> 0;
    g = (g + gg) >>> 0;
    h = (h + hh) >>> 0;
  }
  return [a, b, c, d, e, f, g, h]
    .map((value) => value.toString(16).padStart(8, "0"))
    .join("");
}
