...
test("approval expiry requires an authorized system actor and ignores spoofed identity", async () => {
  let received;
  const app = createRoomsApp({
    auth: async () => ({
      tenantId: "tenant-1",
      actor: {
        ...
        actorType: "system",
        roles: ["approval.expire"],
      },
    }),
    service: fakeService({
      expireApproval: async (...args) => {
        received = args;
        return { status: "expired" };
      },
    }),
  });
  const response = await app.request("/approvals/approval-1/expire", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({
      expiredBy: "spoofed",
      expectedExpiresAt: "2026-07-14T12:01:00.000Z",
    }),
  });
  assert.equal(response.status, 200);
  assert.deepEqual(received, [
  ...
  const forbidden = await createRoomsApp({ service: fakeService() }).request(
    "/approvals/approval-1/expire",
    {
      method: "POST",
