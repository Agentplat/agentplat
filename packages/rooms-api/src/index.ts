...
type ExpireApprovalInput = NonNullable<
  Parameters<RoomService["expireApproval"]>[2]
>;
...
export interface CreateRoomsAppOptions {
  ...
  planner?: Pick<
    AgentRoomPlannerBridge,
    "create" | "get" | "materialize" | "replanFromEvent" | "reconcileFromEvent"
  >;
...
export function createRoomsApp(
  ...
  app.post("/approvals/:approvalId/expire", async (context) => {
    const input = await readOptionalJsonObject<ExpireApprovalInput>(context);
    const approval = await options.service.expireApproval(
      tenantId(context),
      context.req.param("approvalId"),
      trustedApprovalExpiryInput(context, input),
    );
    return context.json({ data: approval });
  });
  ...
  if (options.liveView) {
    const liveInput = (context: Context<RoomsApiEnv>) => ({
      ...
      executionSessionIds: csvOptional(
        context.req.query("executionSessionIds"),
      ),
...
function trustedApprovalExpiryInput(
  context: Context<RoomsApiEnv>,
  input: ExpireApprovalInput,
): ExpireApprovalInput {
  const actor = context.get("tenant").actor;
  if (
    !actor ||
    !["machine", "system"].includes(actor.actorType) ||
    !actor.roles?.some((role) => role === "*" || role === "approval.expire")
  ) {
    throw new AgentPlatError(
      "FORBIDDEN",
      "Approval expiry requires an authorized machine or system actor",
      { statusCode: 403 },
    );
  }
  const sanitized = omitReserved(
  ...
  return actor.actorId ? { ...sanitized, expiredBy: actor.actorId } : sanitized;
}
