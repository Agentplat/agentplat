// Persistent reference assembly. No model calls or external provider effects.
...
import { RoomService } from "../../packages/rooms/dist/index.js";
import { PostgresRoomRepository, runMigrations as migrateRooms } from "../../packages/rooms-postgres/dist/index.js";
...
import { createAgentRoomMorphogenesisGateConfigurationV1, createAgentRoomMorphogenesisDecisionAuthorizationV1 } from "../../packages/workflows-rooms/dist/morphogenesis.js";
import { projectMorphogenesisSupersessionToRoomArtifactV1, projectMorphogenesisReceiptToRoomArtifactV1 } from "../../packages/rooms-mesh/dist/morphogenesis.js";
import { fixture } from "./persistent-fixture.mjs";

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../..");
const sha = c => `sha256:${c.repeat(64)}`;
const digest = x => typeof x === "string" || Buffer.isBuffer(x)
  ? `sha256:${createHash("sha256").update(x).digest("hex")}`
  : C.digestCollectiveJsonV1("state", JSON.parse(JSON.stringify(x)));
...
// Example-owned Team persistence adapter. Domain validation remains in TeamFormationRuntime.
class PgTeamStore {
  constructor(pool, schema) {
    assert(/^[a-z][a-z0-9_]{0,62}$/.test(schema), "invalid example schema");
    this.pool = pool; this.schema = schema;
  }
  async load(key) {
    const r = await this.pool.query(`SELECT state FROM "${this.schema}".reference_team_states WHERE state_key=$1`, [key]);
    return r.rows[0]?.state ?? null;
  }
  async save({state, expectedRevision}) {
    const r = expectedRevision === null
      ? await this.pool.query(`INSERT INTO "${this.schema}".reference_team_states(state_key, revision, state) VALUES($1,$2,$3::jsonb) ON CONFLICT DO NOTHING`, [state.stateKey,state.revision,JSON.stringify(state)])
      : await this.pool.query(`UPDATE "${this.schema}".reference_team_states SET revision=$2,state=$3::jsonb WHERE state_key=$1 AND revision=$4`, [state.stateKey,state.revision,JSON.stringify(state),expectedRevision]);
    return r.rowCount === 1;
  }
}
...
async function setup(pool,schema) {
  ...
  await migrateRooms(pool,{schema});
  const roomRepository=new PostgresRoomRepository(pool,{schema});
  const rooms=new RoomService({repository:roomRepository,eventPublisher:new InMemoryEventBus(),runtime:createMockRuntime(),requireProtectedActionCheckpoints:true,clock:()=>new Date(wall(240))});
  const room=await rooms.createRoom("tenant:test",{id:"room:test",title:"Persistent Morphogenesis",goal:"Preserve work while a specialist transition is superseded"});
  await rooms.addParticipant(room.tenantId,room.id,{id:"agent:planner",type:"agent",displayName:"Local planner",runtime:{platform:"mock"},role:"planner",authorityLevel:0,permissions:[]});
  for(const label of ["a","b","expiry"])await rooms.addParticipant(room.tenantId,room.id,{id:`person:reviewer:${label}`,type:"human",displayName:`Local reviewer ${label}`,role:"reviewer",authorityLevel:1,permissions:["approve:action"]});
  await pool.query(`CREATE TABLE "${schema}".reference_owner_receipts(operation_id text PRIMARY KEY,binding_digest text NOT NULL,receipt jsonb NOT NULL)`);
  await pool.query(`CREATE TABLE "${schema}".reference_jobs(job_id text PRIMARY KEY,work_id text NOT NULL,status text NOT NULL)`);

  await pool.query(`CREATE TABLE "${schema}".reference_team_states(state_key text PRIMARY KEY,revision integer NOT NULL,state jsonb NOT NULL)`);
  await pool.query(`CREATE TABLE "${schema}".reference_work_effects(effect_id text PRIMARY KEY,work_id text NOT NULL,body jsonb NOT NULL)`);
  ...
  return {pool,schema,work,authority,document,heads,headStore,budgets,scopeOptions,options,rooms,room,roomRepository};
}
...
async function transition(env,label,activationTime=300) {
  ...
  const authorizations={
    async issue({candidate}) {
      const gate=createAgentRoomMorphogenesisGateConfigurationV1({candidate,roomId:env.room.id,requestedBy:"agent:planner"});
      const requested=await env.rooms.requestApproval(env.room.tenantId,env.room.id,gate);
      const approval=await env.rooms.resolveApproval(env.room.tenantId,requested.id,"approved",{decidedBy:`person:reviewer:${label}`});
      const authorization=createAgentRoomMorphogenesisDecisionAuthorizationV1({candidate,approval,
        expectedTenantId:env.room.tenantId,expectedRoomId:env.room.id,
        actorMandateDigest:digest({roomId:env.room.id,actorId:approval.decidedBy,permissions:["approve:action"]}),
        independenceGroupId:`reviewer:${label}`,proofDigest:digest(approval),decidedAtLogicalMs:240,expiresAtLogicalMs:470});
      record("room-approval",{approval,candidateDigest:candidate.candidateDigest});
  ...
  const teamPort=new TeamFormationMorphogenesisSuccessorPortV1({formation,commands:{
    ...
    async continueExecution({jointWorkContract}) {
      ...
      await env.pool.query(`INSERT INTO "${env.schema}".reference_work_effects(effect_id,work_id,body) VALUES($1,$2,$3::jsonb) ON CONFLICT DO NOTHING`,
  ...
  // Owner receipts are persisted and bind immutable subjects. Never infer an
  // unapplied effect from a timeout. Reconciliation may reconstruct only an
  // owner state bearing this exact operation's terminal reason.
  const getReceipt=async(input,kind)=>{
    const binding=digest({kind,scopeDigest:input.scope.scopeDigest,proposalDigest:input.proposalDigest,agentDigest:input.agent.agentDigest,teamReceiptDigest:input.team?.receiptDigest??null});
    const row=(await env.pool.query(`SELECT binding_digest,receipt FROM "${env.schema}".reference_owner_receipts WHERE operation_id=$1`,[input.operationId])).rows[0];
    if(row&&row.binding_digest!==binding)throw new Error("owner operation binding changed");
    return {binding,receipt:row?.receipt};
  };
  const saveReceipt=async(input,kind,receipt)=>{
    const {binding}=await getReceipt(input,kind);
    await env.pool.query(`INSERT INTO "${env.schema}".reference_owner_receipts(operation_id,binding_digest,receipt) VALUES($1,$2,$3::jsonb) ON CONFLICT DO NOTHING`,[input.operationId,binding,JSON.stringify(receipt)]);
    return (await getReceipt(input,kind)).receipt;
  };
  const startWork=async()=>{
    ...
    try {
      ...
      const authority=C.validateCollectiveAuthorityStateV1((await client.query(`SELECT state FROM "${env.schema}".collective_authority_states WHERE tenant_id=$1 AND policy_domain_id=$2 FOR SHARE`,[env.options.tenantId,env.options.policyDomainId])).rows[0].state);
      const permitted=C.authorizeDelegationMandateAtV1(authority,{mandateId:env.document.statement.mandateId,mandateDigest:env.document.mandateDigest,at:wall(321)});
      if(!permitted.authorized)throw new Error("work mandate is not current");
      const state=C.validateCollectiveExecutionStateV1((await client.query(`SELECT state FROM "${env.schema}".collective_execution_states WHERE tenant_id=$1 AND policy_domain_id=$2 FOR UPDATE`,[env.options.tenantId,env.options.policyDomainId])).rows[0].state);
      const work=state.workContracts.find(w=>w.workContractId===`work-contract:${label}`);
      if(!work||work.status!=="active")throw new Error("work admission is fenced");
      await client.query(`INSERT INTO "${env.schema}".reference_jobs(job_id,work_id,status) VALUES($1,$2,'running')`,[jobId,work.workContractId]);
      await client.query("COMMIT");record("work-admitted",{label,workContractDigest:work.workContractDigest});
    }catch(error){await client.query("ROLLBACK");throw error;}finally{client.release();}
    let releaseJob;
    const job=new Promise(resolve=>{releaseJob=resolve;}).then(async()=>{
      await env.pool.query(`UPDATE "${env.schema}".reference_jobs SET status='completed' WHERE job_id=$1`,[jobId]);
      record("admitted-work-finished",{label});
    });
    return {job,finish:()=>releaseJob()};
  };
  const checkpoint=async input=>{
    const existing=await getReceipt(input,"checkpoint");if(existing.receipt)return existing.receipt;
    const rows=(await env.pool.query(`SELECT body FROM "${env.schema}".reference_work_effects WHERE work_id=$1`,[`work-contract:${label}`])).rows;
    const receipt=M.createMorphogenesisContinuityReceiptV1({operationId:input.operationId,agentDigest:input.agent.agentDigest,teamReceiptDigest:input.team.receiptDigest,
      checkpointDigest:digest(rows),preservedArtifactDigests:rows.map(r=>digest(r.body)),completedCausalNodeDigests:[],invalidatedCausalClosureDigests:[],completedAtLogicalMs:input.logicalTimeMs});
    return saveReceipt(input,"checkpoint",receipt);
  };
  let loseFenceAck=true;
  const fence=async input=>{
    const existing=await getReceipt(input,"fence");if(existing.receipt)return existing.receipt;
    const prior=(await env.work.read()).workContracts.find(w=>w.workContractId===`work-contract:${label}`);
    const reason=`morphogenesis:${input.operationId}`;
    let after=prior;
    let appliedNow=false;
    if(!(prior.status==="revoked"&&prior.terminalReasonCode===reason)){
      if(!input.team.individualWorkContractDigests.includes(prior.workContractDigest))throw new Error("Work changed since Team activation");
      const changed=await mutateWork(env.work,state=>C.transitionWorkContractV1(state,{workContractId:prior.workContractId,
        expectedGeneration:prior.generation,expectedDigest:prior.workContractDigest,nextStatus:"revoked",terminalReasonCode:reason,logicalTimeMs:input.logicalTimeMs}));
      after=changed.state.workContracts.find(w=>w.workContractId===prior.workContractId);
      appliedNow=true;
    }
    const result={fencedWorkContractDigests:input.team.individualWorkContractDigests,revokedActionGrantDigests:[],
      successorFenceDigests:[after.workContractDigest],effectReceiptDigest:digest({operationId:input.operationId,work:after}),fencedAtLogicalMs:after.updatedAtLogicalMs};
    if(loseFenceAck&&appliedNow){loseFenceAck=false;throw new Error("simulated lost fence acknowledgement");}
    const retained=await saveReceipt(input,"fence",result);record("work-fenced",{label,state:await env.work.read()});
    return retained;
  };
  const detach=async input=>{
    const existing=await getReceipt(input,"detach");if(existing.receipt)return existing.receipt;
    const active=Number((await env.pool.query(`SELECT count(*) AS count FROM "${env.schema}".reference_jobs WHERE work_id=$1 AND status='running'`,[`work-contract:${label}`])).rows[0].count);
    if(active){record("drain-blocked",{label,inFlight:active,operationId:input.operationId});throw new Error("work still draining");}
    const work=(await env.work.read()).workContracts.find(w=>w.workContractId===`work-contract:${label}`);
    if(work.status!=="revoked")throw new Error("detachment requires fenced Work");
    const state=await formation.loadState();
    const cancelled=await formation.cancel({reasonCode:"participant_detached",expectedProposalDigest:state.team.proposal.proposalDigest,logicalTimeMs:input.logicalTimeMs});
    const receipt=M.createMorphogenesisTerminalAgentReceiptV1({operationId:input.operationId,agentDigest:input.agent.agentDigest,disposition:"detached",
      membershipConfigurationDigest:null,membershipEpoch:null,lifecycleReceiptDigest:cancelled.recordDigest??digest(cancelled),terminatedAtLogicalMs:cancelled.updatedAtLogicalMs});
    const retained=await saveReceipt(input,"detach",receipt);record("participant-detached",{label,receipt:retained});return retained;
  };
  const runtimeOptions={store:executionStore,maximumCommitAttempts:8,
  ...
  const runtime=new M.MorphogenesisExecutionRuntimeV1(runtimeOptions);
  ...
  return {label,context,decision,candidate,decisions,runtime,stateKey,formation,executionStore,activationTime,startWork,runtimeOptions};
}
...
async function main() {
  ...
  const poolOptions={host:"127.0.0.1",port,user:process.env.PGUSER??"postgres",database:process.env.PGDATABASE??"postgres",max:6};
  ...
  const summary={sourceCommit:execFileSync("git",["rev-parse","HEAD"],{cwd:root,encoding:"utf8"}).trim(),
    startedAt:new Date().toISOString(),evidenceClass:"PostgreSQL Room, Team and Work reference assembly; local catalog and scripted reviewers",externalSpendUsd:0,modelCalls:0};
  try {
    ...
    const before={head:await env.headStore.load("head:paper"),work:await env.work.read(),teams:await Promise.all([a,b].map(t=>t.formation.loadState())),
      effects:(await pool.query(`SELECT * FROM "${schema}".reference_work_effects ORDER BY effect_id`)).rows};
    ...
    await loser.runtime.beginSupersededResolution({stateKey:loser.stateKey,logicalTimeMs:320});
    await loser.runtime.advanceSupersededResolution({stateKey:loser.stateKey,logicalTimeMs:320});
    await assert.rejects(loser.runtime.advanceSupersededResolution({stateKey:loser.stateKey,logicalTimeMs:320}),/lost fence acknowledgement/);
    const retainedPending=await loser.runtime.required(loser.stateKey);
    assert.equal(retainedPending.phase,"fencing");
    const recovered=new M.MorphogenesisExecutionRuntimeV1(loser.runtimeOptions);
    let resolved=retainedPending;
    for(let step=0;step<5&&resolved.phase!=="superseded";step++)resolved=await recovered.advanceSupersededResolution({stateKey:loser.stateKey,logicalTimeMs:321});
    assert.equal(resolved.phase,"superseded");
    const projection=projectMorphogenesisSupersessionToRoomArtifactV1({room:env.room,execution:resolved});
    const roomArtifact=await env.rooms.createArtifact(env.room.tenantId,env.room.id,projection.input);
    const cleanup={work:await env.work.read(),team:await loser.formation.loadState(),execution:resolved,retainedPending,roomArtifact};
    assert.equal(cleanup.work.workContracts.find(w=>w.workContractId===`work-contract:${loser.label}`).status,"revoked");
    ...
    summary.race={approvedProposals:2,teamsActivatedBeforeHead:before.teams.filter(t=>t.team.status==="active").length,
      ...
      losingWorkRevoked:true,losingTeamCancelled:true,losingExecutionPhase:cleanup.execution.phase,newPoolReopenVerified:true,roomArtifactId:roomArtifact.id,fenceAcknowledgementRecovered:true};
    const admitted=await winner.startWork();
    await winner.runtime.checkpoint({stateKey:winner.stateKey,logicalTimeMs:325});
    await assert.rejects(winner.runtime.fenceAuthority({stateKey:winner.stateKey,logicalTimeMs:330}),/lost fence acknowledgement/);
    await winner.runtime.fenceAuthority({stateKey:winner.stateKey,logicalTimeMs:331});
    ...
    const completedProjection=projectMorphogenesisReceiptToRoomArtifactV1({room:env.room,receipt:terminal.receipt});
    await env.rooms.createArtifact(env.room.tenantId,env.room.id,completedProjection.input);
    const roomState=await env.roomRepository.getRoomState(env.room.tenantId,env.room.id);
    assert.equal(roomState.approvals.filter(a=>a.status==="approved").length,2);
    assert(roomState.artifacts.some(a=>a.id===roomArtifact.id));
    write(path.join(output,"room.json"),roomState);
    ...
    write(path.join(output,"environment.json"),{node:process.version,platform:process.platform,arch:process.arch,
      trackedDiffDigest:digest(execFileSync("git",["diff","HEAD","--"],{cwd:root})),
      ...
      sourceFiles: ["persistent.mjs","persistent-fixture.mjs"].map(f=>({path:f,digest:digest(readFileSync(new URL(f,import.meta.url)))})),
