...
try{
  ...
  started=true;
  for(let i=0;i<100;i++){
    const p=spawnSync("docker",["exec",name,"pg_isready","-h","127.0.0.1","-U","postgres"],{encoding:"utf8"});
    if(p.status===0)break;
  ...
  const child=spawnSync(process.execPath,[fileURLToPath(new URL("persistent.mjs",import.meta.url)),path.resolve(output)],{
