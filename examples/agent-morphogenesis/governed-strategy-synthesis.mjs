...
const policy = createMorphogenesisStrategySynthesisPolicyV5({
  schemaVersion: 5,
  policyId: "policy:synthesis:example",
  policyVersion: 1,
  ...
  maximumCandidateTtlMs: 60_000,
  maximumEvaluationTtlMs: 30_000,
...
const gap = createMorphogenesisStrategyGapV5({
  ...
  governanceStateDigest: digest("governance-state"),
...
const manifest = createMorphogenesisSynthesizedStrategyManifestV5({
  ...
  toolSetDigest: digest("tool-set"),
  memoryScopeDigest: digest("memory-scope"),
  ...
  synthesizer: {
    ...
    async synthesize() {
      return {
        ...
        manifest,
        provenanceDigests: [digest("generation-receipt")],
  ...
  JSON.stringify(
    {
      ...
      next: "simulate, independently certify, gate, review, then canary",
    },
    null,
    2,
  ),
