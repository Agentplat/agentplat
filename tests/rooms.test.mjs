...
test("approval expiry is terminal, replay-safe and never auto-approves", async () => {
  let now = new Date("2026-07-14T12:00:00.000Z");
  const { service } = fixture({ clock: () => now });
  const { room, human } = await createRoomTeam(service);
  const artifact = await service.createArtifact("tenant-a", room.id, {
    type: "brief",
    title: "Expiring brief",
    content: "draft",
  });
  const approval = await service.requestApproval("tenant-a", room.id, {
    targetType: "artifact",
  ...
  assert.equal(approval.expiresAt, "2026-07-14T12:01:00.000Z");
  await assert.rejects(
    service.expireApproval("tenant-a", approval.id, {
  ...
  await assert.rejects(
    ...
    (error) => error.code === "CONFLICT" && /expired/.test(error.message),
  );
  ...
  assert.equal(expired.expiredAt, "2026-07-14T12:01:00.000Z");
  assert.equal(expired.expiredBy, "expiry-worker");
  assert.deepEqual(
    await service.expireApproval("tenant-a", approval.id, {
      expiredBy: "another-worker",
      expectedExpiresAt: approval.expiresAt,
    }),
    expired,
  );
  const state = await service.getRoomState("tenant-a", room.id);
  assert.equal(
    state.events.filter((event) => event.type === "approval_expired").length,
    1,
  );
  assert.equal(state.artifacts[0].status, "pending_approval");
});

test("a human decision before the deadline wins over later expiry", async () => {
  let now = new Date("2026-07-14T12:00:00.000Z");
  const { service } = fixture({ clock: () => now });
  const { room, human } = await createRoomTeam(service);
  const approval = await service.requestApproval("tenant-a", room.id, {
