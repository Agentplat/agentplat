...
const sha = (value) =>
  digestPlanningJsonV1("morphogenesis-strategy-context-v3", { value });

function synthesizedProfile() {
  const materialBody = Object.freeze({
    schemaVersion: 1,
    profileId: "profile:genesis:material",
    profileVersion: 1,
    ...
    roleDefinitionDigest: sha("role"),
    roleCertificationDigest: sha("role-certification"),
    capabilityKeys: ["incident_analysis"],
    toolNames: ["logs.read"],
    actionClasses: ["read"],
    adapterId: "adapter:sandbox",
    adapterVersion: "1",
    instructionArtifactId: "artifact:instructions",
    instructionArtifactDigest: sha("instructions"),
    toolSetArtifactId: "artifact:tools",
    toolSetArtifactDigest: sha("tools"),
    memoryScopeId: "memory:probation",
    memoryScopeDigest: sha("memory"),
    inputContractDigest: sha("input"),
    outputContractDigest: sha("output"),
    modelConstraintsDigest: sha("model"),
    authorityCeilingDigest: sha("authority"),
    localRuleProgramDigest: sha("rules"),
    resourceBudgetUnits: 10,
    interactionBudgetUnits: 20,
    maximumActionBudgetUnits: 2,
    requiredAssessorIds: ["assessor:capability"],
    requiredAttestationDigests: [sha("attestation")],
    authorId: "agent:generator",
    provenanceDigest: sha("provenance"),
    validFromLogicalMs: 10,
    expiresAtLogicalMs: 100,
  });
  const materialProfile = Object.freeze({
    ...materialBody,
    profileDigest: digestPlanningJsonV1(
  ...
  const evolution = createAgentInstantiationProfileEvolutionV1({
    ...
    evidenceDigests: [sha("evolution-evidence")],
    proposedAtLogicalMs: 15,
  });
  const attenuation = createAgentInstantiationAuthorityAttenuationV1({
    attenuationId: "attenuation:genesis",
    evolutionDigest: evolution.evolutionDigest,
    parentAuthorityCeilingDigests: [],
    childAuthorityCeilingDigest: sha("authority"),
    retainedToolNames: ["logs.read"],
    removedToolNames: [],
    retainedActionClasses: ["read"],
    removedActionClasses: [],
    policyDigest: sha("attenuation-policy"),
    issuerId: "agent:attenuator",
    issuerImplementationDigest: sha("attenuator"),
    evidenceDigests: [sha("attenuation")],
    issuedAtLogicalMs: 16,
    validUntilLogicalMs: 90,
  });
  const synthesisCertification =
    createAgentInstantiationSynthesisCertificationV1({
      certificationId: "certification:genesis",
      evolutionDigest: evolution.evolutionDigest,
      materialProfileDigest: materialProfile.profileDigest,
      synthesizerId: "agent:generator",
      ...
      evidenceDigests: [sha("certification")],
      certifiedAtLogicalMs: 17,
...
function fixture(maximumResourceBudgetUnits = 20) {
  ...
  const policy = createMorphogenesisAgentGenesisPolicyV6({
    ...
    admittedGeneratorImplementationDigests: [
      synthesized.generatorImplementationDigest,
    ],
  ...
  const generator = {
    generatorId: "agent:generator",
    generatorVersion: 1,
    generatorImplementationDigest: synthesized.generatorImplementationDigest,
    async generate() {
      return {
        draftId: "draft:agent-genesis",
        profile: synthesized.profile,
        profileContext: synthesized.profileContext,
        modelBindingDigest: sha("model-binding"),
        provenanceDigests: [sha("generation")],
        maximumSpawnDepth: 0,
...
test("V6 rejects eligible catalog blueprints, foreign generators and budget widening", async () => {
  ...
  assert.throws(
    () =>
      createMorphogenesisAgentGenesisNeedV6({
        ...value.need,
        eligibleBlueprintDigests: [sha("catalog-blueprint")],
        policy: value.policy,
      }),
    /eligible blueprint/,
  );
  assert.throws(
    () =>
      new MorphogenesisAgentGenesisRuntimeV6({
        policy: value.policy,
        generator: {
          ...value.generator,
          generatorImplementationDigest: sha("foreign"),
        },
      }),
    /not admitted/,
  );
  ...
  await assert.rejects(
    ...
    }).generate({ need: constrained.need, logicalTimeMs: 20 }),
...
test("V6 requires complete adversarial evidence before authority-free sandbox preparation", async () => {
  ...
  const threatAssessments = MORPHOGENESIS_AGENT_GENESIS_THREATS_V6.map(
    (threat) =>
      createMorphogenesisAgentGenesisThreatAssessmentV6({
        threat,
        disposition: "passed",
  ...
  assert.throws(
    () =>
      createMorphogenesisAgentGenesisEvaluationV6({
        ...
        draft,
        baselineBlueprintDigest: sha("baseline"),
        simulatorImplementationDigest: sha("simulator"),
        environmentDigest: sha("environment"),
        seedDigest: sha("seed"),
        threatAssessments: threatAssessments.slice(1),
        safetyMicros: 900_000,
        confidenceBps: 9_000,
        interactionUnits: 5,
        assessorId: "agent:assessor",
        assessorImplementationDigest: sha("assessor"),
        evidenceDigests: [sha("evaluation")],
  ...
  const evaluation = createMorphogenesisAgentGenesisEvaluationV6({
    evaluationId: "evaluation:genesis",
    draft,
    baselineBlueprintDigest: sha("baseline"),
    simulatorImplementationDigest: sha("simulator"),
    environmentDigest: sha("environment"),
  ...
  const sandbox = {
    ...
    async prepare({ operationId, draft, logicalTimeMs }) {
      const body = Object.freeze({
        schemaVersion: 6,
        operationId,
        draftDigest: draft.draftDigest,
        profileDigest: draft.profile.profileDigest,
        sandboxId: "sandbox:genesis",
        sandboxImplementationDigest,
        isolationPolicyDigest: sha("isolation"),
        preparedAtLogicalMs: logicalTimeMs,
        expiresAtLogicalMs: 55,
      ...
      return Object.freeze({
        ...
        receiptDigest: digestPlanningJsonV1(
          "morphogenesis-agent-genesis-sandbox-receipt-v6",
          body,
        ),
      });
    },
  };
  const receipt = await new MorphogenesisAgentGenesisSandboxRuntimeV6({
    sandbox,
  }).prepare({
...
test("V6 durably advances sandbox and probation through agent, person or quorum review", async () => {
  ...
  const evaluation = createMorphogenesisAgentGenesisEvaluationV6({
    evaluationId: "evaluation:lifecycle",
    draft,
    baselineBlueprintDigest: sha("baseline"),
    simulatorImplementationDigest: sha("simulator"),
    environmentDigest: sha("environment"),
  ...
  let effects = 0;
  const sandbox = new MorphogenesisAgentGenesisSandboxRuntimeV6({
    sandbox: {
      sandboxImplementationDigest,
      async prepare({ operationId, draft, logicalTimeMs }) {
        const retained = sandboxReceipts.get(operationId);
        ...
        const body = Object.freeze({
          ...
          draftDigest: draft.draftDigest,
          profileDigest: draft.profile.profileDigest,
          sandboxId: `sandbox:${operationId}`,
          sandboxImplementationDigest,
          isolationPolicyDigest: sha("isolation"),
          preparedAtLogicalMs: logicalTimeMs,
          expiresAtLogicalMs: 65,
          membershipGranted: false,
          workGranted: false,
          actionAuthorityGranted: false,
        });
        ...
        sandboxReceipts.set(operationId, receipt);
        return receipt;
      },
    },
  });
  const lifecyclePolicy = createMorphogenesisAgentGenesisLifecyclePolicyV6({
    schemaVersion: 6,
    policyId: "policy:genesis-lifecycle",
    policyVersion: 1,
    ...
    maximumProbationObservations: 4,
    minimumProbationObservations: 2,
    minimumProbationSuccesses: 2,
  ...
  for (const [route, actorType] of [
    ...
    const lifecycle = {
      async createAndEnroll({ operationId, profile }) {
        const retained = agents.get(operationId);
        if (retained) return retained;
        const agent = createMorphogenesisLifecycleAgentV1({
          agentId: `agent:${route}`,
          peerId: `peer:${route}`,
          instanceId: `instance:${route}`,
          lineageDigest: sha(`lineage:${route}`),
          capabilityKeys: profile.capabilityKeys,
          roleDefinitionDigest: profile.roleDefinitionDigest,
          membershipConfigurationDigest: sha(`membership:${route}`),
          membershipEpoch: 1,
          source: "synthesized_created",
        });
        agents.set(operationId, agent);
        return agent;
      },
    ...
    const attestation = {
      async attest({ operationId, agent, profile, logicalTimeMs }) {
        const retained = attestations.get(operationId);
        if (retained) return retained;
        const receipt = createMorphogenesisAgentAttestationV1({
          operationId,
          ...
          validUntilLogicalMs: 65,
        });
        attestations.set(operationId, receipt);
        return receipt;
      },
    ...
    const retirement = {
      async retire({ operationId, agent, logicalTimeMs }) {
        const retained = terminal.get(operationId);
        if (retained) return retained;
        retirementEffects += 1;
        const receipt = createMorphogenesisTerminalAgentReceiptV1({
          operationId,
          agentDigest: agent.agentDigest,
          disposition: "retired",
          membershipConfigurationDigest: sha(`membership:retired:${route}`),
          membershipEpoch: 2,
          lifecycleReceiptDigest: sha(`lifecycle:retired:${route}`),
          terminatedAtLogicalMs: logicalTimeMs,
        });
        terminal.set(operationId, receipt);
        return receipt;
      },
      async reconcile(input) {
    ...
    const runtime = new MorphogenesisAgentGenesisLifecycleRuntimeV6({
      ...
      reviews: {
        async review({ recommendation, logicalTimeMs }) {
          reviews += 1;
          return createMorphogenesisAgentGenesisReviewV6({
            ...
            proofDigest: sha(`proof:${route}:${reviews}`),
    ...
    const recommend = (id, action, time) =>
      runtime.recommend({
    ...
    const probationPort = (source) => ({
      source,
      async assess({ draft, sandboxReceipt, logicalTimeMs }) {
        return createMorphogenesisAgentGenesisProbationAssessmentV6({
          source,
          draftDigest: draft.draftDigest,
          sandboxReceiptDigest: sandboxReceipt.receiptDigest,
          disposition: "eligible",
          policyDigest: sha(`probation-policy:${source}`),
    ...
    const probationGate =
      new MorphogenesisAgentGenesisProbationEligibilityGateV6({
        capability: probationPort("capability"),
        trust: probationPort("trust"),
    ...
    if (route === "authorized_agent") {
      const restrictedTrust = {
        ...
        async assess({ draft, sandboxReceipt, logicalTimeMs }) {
          return createMorphogenesisAgentGenesisProbationAssessmentV6({
            ...
            policyDigest: sha("probation-policy:trust"),
      ...
      const restricted =
        await new MorphogenesisAgentGenesisProbationEligibilityGateV6({
          capability: probationPort("capability"),
          trust: restrictedTrust,
          inferenceControl: probationPort("inference_control"),
        }).evaluate({
          draft,
          sandboxReceipt: probationEntry.sandboxReceipt,
          logicalTimeMs: 26,
        });
      assert.equal(restricted.disposition, "ineligible");
      await assert.rejects(
        runtime.observeProbation({
          observationId: "observation:restricted",
          draftDigest: draft.draftDigest,
          outcome: "success",
    ...
    for (let index = 1; index <= 2; index += 1) {
      ...
      const eligibility = await probationGate.evaluate({
        draft,
        sandboxReceipt: probationEntry.sandboxReceipt,
        logicalTimeMs,
      });
      await runtime.observeProbation({
        observationId: `observation:${route}:${index}`,
        draftDigest: draft.draftDigest,
        outcome: "success",
        eligibility,
        logicalTimeMs,
    ...
    await runtime.applyMembership({
      operationId: `membership:${route}`,
      draftDigest: draft.draftDigest,
      scope,
      proposalDigest: sha(`proposal:${route}`),
      logicalTimeMs: 34,
    });
    await runtime.attestAdmission({
      operationId: `attest:${route}`,
      draftDigest: draft.draftDigest,
      scope,
      proposalDigest: sha(`proposal:${route}`),
      logicalTimeMs: 35,
    ...
    const meshRecommendation =
      await projectMorphogenesisAgentGenesisRecommendationToMeshV6({
        scope,
        recommendation,
      });
    ...
    await new MorphogenesisAgentGenesisMeshPublisherV6({
      async send(projection) {
        ...
        return {
          ...
          membershipConfigurationDigest: sha("mesh-membership"),
          membershipEpoch: 1,
    ...
    await assert.rejects(
      recommend("late-rollback", "rollback", 36),
      /not allowed/,
    );
    ...
    await runtime.compensateAdmission({
      operationId: `compensate:${route}`,
      draftDigest: draft.draftDigest,
      scope,
      proposalDigest: sha(`proposal:${route}`),
      fence,
      reasonCode: "genesis_probation_retired",
      logicalTimeMs: 39,
    });
    await runtime.compensateAdmission({
      operationId: `compensate:${route}`,
    ...
    const lineage = createMorphogenesisAgentGenesisLineageV6({
      lineageId: `lineage:genesis:${route}`,
      entry: retired,
      logicalTimeMs: 39,
    });
    assert.equal(lineage.grantsAuthority, false);
    assert.equal(
      lineage.terminalReceiptDigest,
      retired.terminalReceipt.terminalReceiptDigest,
    );
    const state = await runtime.state(39);
    assert.equal(
      validateMorphogenesisAgentGenesisLifecycleStateV6(state, {
        policy: lifecyclePolicy,
        genesisPolicy: value.policy,
      }).entries[0].status,
      "retired",
    ...
    assert.throws(
      () =>
        validateMorphogenesisAgentGenesisLifecycleStateV6(
          {
            ...state,
            entries: [{ ...state.entries[0], workGranted: true }],
          },
