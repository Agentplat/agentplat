...
test("registry consumer pins all 56 public packages to the exact release version", () => {
  ...
  assert.equal(REGISTRY_PACKAGES.length, 56);
...
test("registry consumer copies compatibility and conformance scenarios", () => {
  assert.deepEqual(REGISTRY_CONSUMER_SCRIPTS, [
    ...
    {
      source: "scripts/pack-consumers/collective-quorum-crypto.mjs",
      destination: "verify-collective-quorum-crypto.mjs",
    },
  ]);
