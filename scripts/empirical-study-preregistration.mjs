...
async function plan() {
  ...
  const source = await readJson(path.join(operationDirectory, "source-lock.json"));
  const campaign = await readJson(path.join(operationDirectory, "registration.json"));
  const adapter = await readJson(path.join(operationDirectory, "adapter-descriptor.json"));
  const operationPlan = await readJson(path.join(operationDirectory, "operation-plan.json"));
  const expectedManifest = await readJson(path.join(operationDirectory, "expected-manifest.json"));
  const estimate = await readJson(path.join(operationDirectory, "estimate.json"));
  ...
  assert(operationPlan.adapterClass === "diagnostic", "empirical_plan_adapter_must_be_unregistered");
  assert(adapter.runnerClass === "diagnostic", "empirical_adapter_must_be_unregistered");
  assert(adapter.capabilities.exactReplay === false, "empirical_adapter_must_not_claim_replay");
  assert(adapter.capabilities.evaluatorOwnedMetrics === false, "empirical_adapter_must_not_claim_evaluator");
  assert(estimate.executionPermitted === false, "empirical_estimate_must_not_permit_execution");
  const documentBytes = await readFile(path.join(root, protocol.protocolDocument));
  const body = {
    schemaVersion: 1,
  ...
  status({
    ...
    maximumExternalSpend: registration.protocol.budgetPolicy.maximumExternalSpend,
    executionPermitted: false,
  });
}
...
async function resultsTemplate() {
  ...
  const body = {
    ...
    endpointResults: registration.protocol.primaryEndpoints.map((endpointId) => ({
      schemaVersion: 1,
      endpointId,
      status: "not_reported",
      numerator: null,
...
async function verifyAttestation() {
  ...
  assert(value.algorithm === "Ed25519", "empirical_attestation_algorithm_invalid");
  assert(value.issuerId === tokenOption("issuer-id"), "empirical_attestation_issuer_mismatch");
  assert(value.keyId === textOption("key-id"), "empirical_attestation_key_mismatch");
  const registration = validateRegistration(value.registration);
  assert(value.signedDigest === registration.registrationDigest, "empirical_signed_digest_mismatch");
  ...
  assert(value.publicKeyFingerprint === sha256(publicDer), "empirical_public_key_fingerprint_mismatch");
  ...
  assert(
    attestationDigest === digest("empirical-study-registration-attestation-v1", body),
...
function validateProtocol(value) {
  ...
  assert(value.kind === "agentplat-empirical-study-protocol-v1", "empirical_protocol_kind_invalid");
  assert(value.protocolId === "agentplat-collective-control-empirical-study-v1", "empirical_protocol_id_invalid");
  assert(canonical(value.treatments) === canonical(["adaptive_collective", "centralized_planner"]), "empirical_treatments_invalid");
  assert(canonical(value.scales) === canonical([50, 100, 250, 500]), "empirical_scales_invalid");
  assert(canonical(value.strata) === canonical(["nominal", "benign", "adversarial", "mixed"]), "empirical_strata_invalid");
  assert(Array.isArray(value.hypotheses) && value.hypotheses.length === 5, "empirical_hypotheses_invalid");
  for (const hypothesis of value.hypotheses) {
    exactKeys(hypothesis, ["id", "primaryEndpointIds", "statement"], "empirical_hypothesis_shape_invalid");
    assert(typeof hypothesis.statement === "string" && hypothesis.statement.length > 0, "empirical_hypothesis_statement_invalid");
    assert(Array.isArray(hypothesis.primaryEndpointIds) && hypothesis.primaryEndpointIds.length > 0, "empirical_hypothesis_endpoints_invalid");
  }
  assert(canonical(value.hypotheses.map((item) => item.id)) === canonical(["H1", "H2", "H3", "H4", "H5"]), "empirical_hypothesis_ids_invalid");
  assert(Array.isArray(value.primaryEndpoints) && value.primaryEndpoints.length === 7, "empirical_endpoints_invalid");
  exactKeys(value.analysisPolicy, [
  ...
  assert(value.analysisPolicy.bootstrapResamples === 10_000, "empirical_bootstrap_policy_invalid");
  assert(value.analysisPolicy.familywiseMethod === "holm", "empirical_familywise_policy_invalid");
  assert(value.analysisPolicy.maximumSafetyViolations === 0, "empirical_safety_policy_invalid");
  assert(value.budgetPolicy.currency === "USD" && value.budgetPolicy.maximumExternalSpend === 0, "empirical_budget_policy_invalid");
  for (const key of ["paidModelCallsPermitted", "cloudComputePermitted", "managedDatabasePermitted", "dataEgressSpendPermitted"])
    assert(value.budgetPolicy[key] === false, "empirical_budget_policy_invalid");
  assert(value.claimBoundary.empiricalResultClaimPermitted === false, "empirical_claim_boundary_invalid");
  assert(value.claimBoundary.deploymentAuthorityGranted === false, "empirical_claim_boundary_invalid");
  assert(value.claimBoundary.productionReadinessClaimPermitted === false, "empirical_claim_boundary_invalid");
  assert(value.claimBoundary.securityCertificationClaimPermitted === false, "empirical_claim_boundary_invalid");
  assert(value.protocolDocument === "docs/collective-runtime/empirical-validation-protocol-v1.md", "empirical_protocol_document_invalid");
...
function validateRegistration(value) {
  ...
  assert(value.kind === "agentplat-empirical-study-registration-v1", "empirical_registration_kind_invalid");
  const normalizedProtocol = validateProtocol(value.protocol);
  assert(canonical(normalizedProtocol) === canonical(protocol), "empirical_protocol_not_repository_version");
  assert(value.protocolDigest === digest("empirical-study-protocol-v1", normalizedProtocol), "empirical_protocol_digest_mismatch");
  assert(/^[a-f0-9]{40}$/u.test(value.sourceCommit), "empirical_source_commit_invalid");
  for (const key of ["protocolDocumentDigest", "sourceTreeDigest", "campaignRegistrationDigest", "operationPlanDigest", "expectedManifestDigest", "capacityEstimateDigest"])
    requiredDigest(value[key]);
  assert(value.expectedCells === 240, "empirical_expected_cells_invalid");
  assert(value.expectedExecutionSlots === 960, "empirical_expected_slots_invalid");
  assert(value.maximumRegisteredInteractions === 3_296_000, "empirical_interaction_ceiling_invalid");
  assert(value.resultsStatus === "not_collected", "empirical_results_status_invalid");
  assert(value.executionPermitted === false, "empirical_execution_must_be_forbidden");
  const { registrationDigest, ...body } = value;
  assert(registrationDigest === digest("empirical-study-registration-v1", body), "empirical_registration_digest_mismatch");
  return structuredClone(value);
}

async function kmsPublicKey(requestedKeyId) {
  const response = awsJson(["kms", "get-public-key", "--key-id", requestedKeyId]);
  assert(response.KeySpec === "ECC_NIST_EDWARDS25519", "empirical_kms_key_spec_invalid");
  assert(response.KeyUsage === "SIGN_VERIFY", "empirical_kms_key_usage_invalid");
  assert(response.SigningAlgorithms?.includes("ED25519_SHA_512"), "empirical_kms_algorithm_invalid");
  const publicKeyBytes = Buffer.from(requiredText(response.PublicKey), "base64");
  await globalThis.crypto.subtle.importKey("spki", publicKeyBytes, { name: "Ed25519" }, false, ["verify"]);
  return { keyId: requiredText(response.KeyId), publicKeyBytes };
}

function kmsSign(keyId, message) {
  const response = awsJson([
    "kms", "sign", "--key-id", keyId, "--message", message.toString("base64"),
...
function awsJson(args) {
  const prefix = [];
  if (options["aws-profile"]) prefix.push("--profile", textOption("aws-profile"));
...
function pemPublicBytes(value) {
  assert(value.includes("-----BEGIN PUBLIC KEY-----") && value.includes("-----END PUBLIC KEY-----"), "empirical_public_key_pem_invalid");
  const encoded = value.replace(/-----BEGIN PUBLIC KEY-----|-----END PUBLIC KEY-----|\s/gu, "");
...
function parseOptions(args) {
  ...
  for (let index = 0; index < values.length; index += 2) {
    ...
    if (!name?.startsWith("--") || value === undefined || value.startsWith("--")) fail("empirical_option_syntax_invalid");
...
function exactOptions(expected) {
  ...
  assert(canonical(actual) === canonical(wanted), "empirical_option_set_invalid");
}

function textOption(name) {
  const value = options[name];
  assert(typeof value === "string" && value.length > 0 && value.length <= 512 && value.trim() === value, `empirical_${name}_invalid`);
  return value;
}

function tokenOption(name) {
  const value = textOption(name);
  assert(/^[A-Za-z0-9][A-Za-z0-9._:@/-]{0,127}$/u.test(value), `empirical_${name}_invalid`);
  return value;
}
...
function exactKeys(value, keys, reason) {
  ...
  assert(canonical(Object.keys(value).sort()) === canonical([...keys].sort()), reason);
}

function requiredText(value) {
  assert(typeof value === "string" && value.length > 0, "empirical_text_invalid");
  return value;
}

function requiredDigest(value) {
  assert(typeof value === "string" && /^sha256:[a-f0-9]{64}$/u.test(value), "empirical_digest_invalid");
  return value;
}
