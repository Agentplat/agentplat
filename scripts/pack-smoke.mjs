  ...
  workspaceWrites.push(
    ...
    writeFile(
      ...
      [
        ...
        "if (!service || result.output !== 'ok' || sessionResult.turnsCompleted !== 2 || !response.headers.get('content-type')?.startsWith('text/event-stream') || MESH_CONFORMANCE_REPORT_SCHEMA_VERSION !== 1 || MESH_CONFORMANCE_CASES.length < 1) process.exit(1);",
        '',
      ].join('\n')
    ),
    writeFile(
