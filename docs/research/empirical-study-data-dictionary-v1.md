...
The V2 local execution package produces five immutable result files only after
exact 48-shard/960-projection closure. `raw-rows.json` is the canonical paired
row representation. `paper-dataset.csv` is a rectangular convenience export,
not a second source of truth. `normative-analysis.json` contains the registered
endpoint calculations; `paper-tables.json` copies those endpoint values for
rendering without adding interpretation; and `collection-manifest.json` binds
all files to the source, registration, plan, adapter, authorization, execution
and receipt root.

CSV empty fields represent JSON `null`, never zero or false. Boolean values are
lowercase `true` or `false`. Each row retains `cell_id`, scale, stratum and seed
so paired treatment observations cannot be mistaken for independent samples.
