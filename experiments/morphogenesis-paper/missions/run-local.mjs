...
const output=process.argv[2],plan=process.argv[3];assert(output&&plan&&!existsSync(output),"supply a new output directory and plan.json");
...
try{
  ...
  const child=spawnSync(process.execPath,[fileURLToPath(new URL("run.mjs",import.meta.url)),path.resolve(output),path.resolve(plan)],{
    stdio:"inherit",timeout:600000,env:{PATH:process.env.PATH,LANG:"en_US.UTF-8",
