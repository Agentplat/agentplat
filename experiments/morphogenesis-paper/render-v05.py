...
def figures():
    ...
    for x,texts in [(0,['Persist intent: operation K','Owner applies K; receipt flushed','Worker killed before response','New worker reconciles K','Recover receipt; advance head']),
                    (pw+20,['Both proposals bind epoch e','Owner effects A and B applied','A commits head e + 1','B loses predecessor CAS','B checkpoints, fences and drains'])]:
    ...
    box(d,0,142,W,40,'Persistent mission context and artifact identities',bold=True)
